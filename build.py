...
def update_blog_config(posts_metadata):
    """Generate blog-config.js file with metadata, preserving manually edited titles"""
    blog_config_path = DOCS_JS_DIR / 'blog-config.js'

    # Read existing config to preserve manually edited titles
    existing_titles = {}
    if blog_config_path.exists():
        try:
            with open(blog_config_path, 'r', encoding='utf-8') as f:
                content = f.read()
                # Extract existing titles using regex
                import re
                pattern = r"id:\s*'([^']+)',\s*date:\s*'[^']+',\s*title:\s*'([^']+)'"
                matches = re.findall(pattern, content)
                existing_titles = {post_id: title for post_id, title in matches}
    ...
    config_content = "// Blog configuration - Auto-generated by build.py\n"
    config_content += "// DO NOT EDIT MANUALLY - This file is overwritten on each build\n"
    config_content += "// Exception: Titles are preserved if manually edited\n\n"
    config_content += "const BLOG_CONFIG = {\n"
    config_content += "  blogFolder: 'blogs/',\n"
    config_content += "  postsMetadata: [\n"

    for post in posts_metadata:
        post_id = post['id']

        # Use existing title if available, otherwise use extracted title
        if post_id in existing_titles:
            title = existing_titles[post_id]
            print(f'  Preserving title for {post_id}: {title[:50]}...')
        else:
            title = post['title'].replace("\\", "\\\\").replace("'", "\\'")
            print(f'  New title for {post_id}: {title[:50]}...')

        config_content += f"    {{ id: '{post_id}', date: '{post['date']}', title: '{title}', images: {json.dumps(post['images'])} }},\n"
    ...
    print(f'\nGenerated {blog_config_path} with {len(posts_metadata)} posts')

def update_hero_config():
    """Generate hero-config.js file from assets/hero folder, preserving manually edited metadata"""
    hero_config_path = DOCS_JS_DIR / 'hero-config.js'
    hero_assets_dir = DOCS_DIR / 'assets' / 'hero'

    # Check if hero assets directory exists
    if not hero_assets_dir.exists():
        print('  No hero assets folder found, skipping hero config generation')
        return

    # Read existing config to preserve manually edited metadata
    existing_metadata = {}
    if hero_config_path.exists():
        try:
            with open(hero_config_path, 'r', encoding='utf-8') as f:
                content = f.read()
                # Extract existing metadata using regex
                import re
                pattern = r"src:\s*'([^']+)',\s*meta:\s*'([^']+)'"
                matches = re.findall(pattern, content)
                existing_metadata = {src: meta for src, meta in matches}
                print(f'  Found {len(existing_metadata)} existing hero images to preserve')
        except Exception as e:
            print(f'  Warning: Could not read existing hero config: {e}')

    # Get all image files from hero folder
    image_extensions = ['.jpg', '.jpeg', '.png', '.gif', '.webp']
    hero_images = []

    for file in sorted(hero_assets_dir.iterdir()):
        if file.suffix.lower() in image_extensions:
            filename = file.name
            src = f'assets/hero/{filename}'

            # Use existing metadata if available, otherwise generate new
            if src in existing_metadata:
                meta = existing_metadata[src]
                print(f'  Preserving metadata for {filename}')
            else:
                # Generate metadata from filename
                meta = f"IMG_REF: {file.stem.upper()}<br>"
                meta += f"FILE: {filename.upper()}"
                print(f'  New hero image: {filename}')
    ...
    config_content = "// Hero image configuration - Auto-generated by build.py\n"
    config_content += "// DO NOT EDIT MANUALLY - This file is overwritten on each build\n"
    config_content += "// Exception: Metadata is preserved if manually edited\n\n"
    config_content += "const HERO_CONFIG = {\n"
    config_content += "  heroFolder: 'assets/hero/',\n"
    config_content += "  heroImages: [\n"

    for img in hero_images:
        config_content += f"    {{ src: '{img['src']}', meta: '{img['meta']}' }},\n"

    config_content += "  ]\n"
    config_content += "};\n"

    # Write complete file
    with open(hero_config_path, 'w', encoding='utf-8') as f:
        f.write(config_content)

    print(f'\nGenerated {hero_config_path} with {len(hero_images)} hero images')
...
def main():
    ...
    # Update blog config
    print('\n3. Generating blog-config.js...')
    update_blog_config(posts_metadata)

    # Update hero config
    print('\n4. Generating hero-config.js...')
    update_hero_config()

...
def update_blog_config(posts_metadata):
    """Generate blog-config.js file with metadata"""
    ...
    for post in posts_metadata:
        # Escape single quotes and backslashes in title
        title = post['title'].replace("\\", "\\\\").replace("'", "\\'")
        config_content += f"    {{ id: '{post['id']}', date: '{post['date']}', title: '{title}', images: {json.dumps(post['images'])} }},\n"
